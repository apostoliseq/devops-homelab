#!/usr/bin/env bash
  ...
  # Start a hidden background task.
  # This loop runs every 60s to refresh the sudo timeout (sudo -n true).
  # It checks if the main script ($$) is still alive (kill -0).
  # If the main script ends, the loop exits to avoid leaving a 'zombie' process.
  while true; do
    sudo -n true
    sleep 60
    kill -0 "$$" || exit
